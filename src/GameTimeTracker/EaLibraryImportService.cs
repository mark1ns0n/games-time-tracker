...
internal sealed class EaLibraryImportService
{
    private static readonly string[] ExcludedExecutableTokens =
    [
        "crash", "diagnostic", "dotnet", "helper", "install", "installer",
        "launcherhelper", "redist", "setup", "support", "unins",
        "unitycrashhandler", "vc_redist", "vcredist"
    ];

    public IReadOnlyList<EaGameCandidate> FindCandidates(IEnumerable<GameProfile> existingGames)
    {
    ...
    private static string GetCandidateKey(EaGameCandidate candidate)
    {
    ...
    private static IEnumerable<EaGameCandidate> FindRegistryCandidates()
    {
    ...
    private static EaGameCandidate? ReadRegistryCandidate(RegistryKey root, string subKeyName)
    {
        try
        {
            ...
            var installDirectory = ReadRegistryString(key, "InstallLocation")
                ?? ReadRegistryString(key, "InstallSource")
            ...
            if (!LooksLikeEaGame(name, publisher, installDirectory, displayIcon, uninstallString))
            {
                return null;
            }
    ...
    private static IEnumerable<EaGameCandidate> FindFolderCandidates()
    {
    ...
    private static IReadOnlyList<string> FindLibraryRoots()
    {
        ...
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "EA Games");
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "EA Games");
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Electronic Arts");
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "Electronic Arts");
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Origin Games");
        AddLibraryRoot(roots, Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "Origin Games");
        ...
        foreach (var drive in drives)
        {
            try
            {
                ...
                roots.Add(Path.Combine(drive.RootDirectory.FullName, "EA Games"));
                roots.Add(Path.Combine(drive.RootDirectory.FullName, "Electronic Arts"));
                roots.Add(Path.Combine(drive.RootDirectory.FullName, "Origin Games"));
            }
            catch
            {
                continue;
    ...
    private static EaGameCandidate? ReadFolderCandidate(string installDirectory)
    {
        try
        {
    ...
    private static bool LooksLikeEaGame(
        string name,
        string publisher,
    ...
    {
        if (IsEaPublisher(publisher))
        {
            return true;
        }

        if (name.Contains("Electronic Arts", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }

        return PathLooksLikeEa(installDirectory) ||
            PathLooksLikeEa(displayIcon) ||
            PathLooksLikeEa(uninstallString);
    }

    private static bool IsEaPublisher(string publisher)
    {
        return publisher.Contains("Electronic Arts", StringComparison.OrdinalIgnoreCase) ||
            publisher.Contains("EA Swiss", StringComparison.OrdinalIgnoreCase) ||
            publisher.Equals("EA", StringComparison.OrdinalIgnoreCase);
    }

    private static bool PathLooksLikeEa(string? value)
    {
        ...
        return value.Contains("EA Games", StringComparison.OrdinalIgnoreCase) ||
            value.Contains("Electronic Arts", StringComparison.OrdinalIgnoreCase) ||
            value.Contains("Origin Games", StringComparison.OrdinalIgnoreCase) ||
            value.Contains("EA Desktop", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsLauncherName(string name)
    {
        return name.Equals("EA app", StringComparison.OrdinalIgnoreCase) ||
            name.Equals("EA Desktop", StringComparison.OrdinalIgnoreCase) ||
            name.Equals("Origin", StringComparison.OrdinalIgnoreCase) ||
            name.Contains("EA Launcher", StringComparison.OrdinalIgnoreCase) ||
            name.Contains("Electronic Arts Launcher", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsSupportDirectory(string path)
    {
        var name = NormalizeForScore(Path.GetFileName(path));
        return name is "redist" or "redistributables" or "support" or "tmp" or "temp" or "installer";
    }
    ...
    private static int ScoreExecutable(string installDirectory, string gameName, string path)
    {
        ...
        if (normalizedRelative.Contains("bin", StringComparison.OrdinalIgnoreCase) ||
            normalizedRelative.Contains("game", StringComparison.OrdinalIgnoreCase) ||
...
internal sealed record EaGameCandidate(
    string SourceId,
    string Name,
    string Source,
    string InstallDirectory,
