...
internal sealed class GogImportDialog : Form
{
    private readonly BindingList<GogImportRow> _rows;
    ...
    public GogImportDialog(IReadOnlyList<GogGameCandidate> candidates)
    {
        _rows = new BindingList<GogImportRow>(candidates
            .Select(candidate => new GogImportRow(candidate))
            .ToList());

        Text = "Import GOG games";
        Width = 920;
        Height = 520;
    ...
    private void ConfigureGrid()
    {
        ...
        {
            Name = nameof(GogImportRow.Import),
            DataPropertyName = nameof(GogImportRow.Import),
            HeaderText = "Import",
            Width = 58
        });
        _grid.Columns.Add(new DataGridViewTextBoxColumn
        {
            Name = nameof(GogImportRow.Name),
            DataPropertyName = nameof(GogImportRow.Name),
            HeaderText = "Name",
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn
        {
            Name = nameof(GogImportRow.Source),
            DataPropertyName = nameof(GogImportRow.Source),
            HeaderText = "Source",
            Width = 90,
            ReadOnly = true
        });
        _grid.Columns.Add(new DataGridViewTextBoxColumn
        {
            Name = nameof(GogImportRow.ProcessName),
            DataPropertyName = nameof(GogImportRow.ProcessName),
            HeaderText = "Process",
            Width = 150
        });
        _grid.Columns.Add(new DataGridViewTextBoxColumn
        {
            Name = nameof(GogImportRow.ExecutablePath),
            DataPropertyName = nameof(GogImportRow.ExecutablePath),
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn
        {
            Name = nameof(GogImportRow.InstallDirectory),
            DataPropertyName = nameof(GogImportRow.InstallDirectory),
    ...
    private void SaveSelection()
    {
        ...
        if (games.Count == 0)
        {
            MessageBox.Show(this, "Select at least one GOG game with a process name.", "GameTimeTracker", MessageBoxButtons.OK, MessageBoxIcon.Warning);
    ...
    private sealed class GogImportRow
    {
        private static readonly Size CachedIconSize = new(128, 128);

        private readonly GogGameCandidate _candidate;

        public GogImportRow(GogGameCandidate candidate)
        {
